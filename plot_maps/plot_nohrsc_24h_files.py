...
nohrsc_delta = timedelta(hours=24)
...
# Determine how many 24-hour periods are in the full duration (e.g., 48h/24h)
segments = int(duration) / int(24) 
...
for j in range(int(segments)):
    ...
    # Open 24-h NOHRSC file 
    #filename_nohrsc = f"{DATA_PATH}/nohrsc/sfav2_CONUS_24h_{date_string}_grid184.grb2"
    filename_nohrsc = f"{DATA_PATH}/nohrsc.{date_string}.24h"
...
for i, loc in enumerate(grid_locs):
    ...
    if grid == 'northeast':   
        ...
        ax.set_extent([-128, -61, 24, 55], crs=ccrs.PlateCarree())
        # Add manual aspect ratio here. 
        # Increase this number (e.g., 1.4) to stretch it more vertically
        ...
        ax.set_extent([-124, -108, 32.0, 49.0], crs=ccrs.PlateCarree())
        # Add manual aspect ratio here. 
        # Increase this number (e.g., 1.4) to stretch it more vertically
        ax.set_aspect(1.25, adjustable='datalim')
