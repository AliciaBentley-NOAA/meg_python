...
with grib2io.open(filename_gfs) as f_gfs:
    ...
    temp_gfsv16_msg = f_gfs.select(shortName='DPT', level='2 m above ground')[0]
...
with grib2io.open(filename_gfsv17) as f_gfsv17:
    ...
    # Select the specific messages we want
    temp_gfsv17_msg = f_gfsv17.select(shortName='DPT', level='2 m above ground')[0]
...
plot_configs = [
        {'data': diff_data,       'cmap': white_center_cmap,      'norm': diff_norm, 'levels': diff_levels, 'title': f'GFSv17 minus GFSv16 2-m Dewpoint (F)\nInitialized: {init_dt.strftime("%Y-%m-%d %HZ")} (F{fhr_str}) | Valid: {valid_dt.strftime("%Y-%m-%d %HZ")}'}
]
