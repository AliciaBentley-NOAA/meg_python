...
with grib2io.open(filename_ecmwf_clean) as f_ecmwf:
	...
	mslp_msg = f_ecmwf.select(shortName='PRES', level='mean sea level')[0]
...
print(f"Min MSLP: {mslp_data.min()}, Max MSLP: {mslp_data.max()}")
...
plot_configs = [
        {'data': mslp_data, 'cmap': 'gist_rainbow', 'norm': mslp_norm, 'levels': mslp_levels, 'title': f'EC-AIFS | Mean Sea Level Pressure (hPa)\nInitialized: {init_dt.strftime("%Y-%m-%d %HZ")} (F{fhr_str}) | Valid: {valid_dt.strftime("%Y-%m-%d %HZ")}'},
]
