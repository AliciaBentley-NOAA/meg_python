...
new_hour = int(fhr)
start_hour = int(fhr)-int(duration)
print(f"start_hour: {start_hour}")
print(f"new_hour: {new_hour}")

# Open ECMWF file and extract parameters from valid date
filename_ecmwf_start = f"{DATA_PATH}/ecmwf_aifs.{pdy}/{cyc}/atmos/{init_YYYY}{init_MM}{init_DD}{init_HH}0000-{start_hour}h-oper-fc.grib2"
...
filename_ecmwf_start_clean = get_uncompressed_grib(filename_ecmwf_start)

with grib2io.open(filename_ecmwf_start_clean) as f_ecmwf:

	precip_start_msg = f_ecmwf.select(shortName='TPRATE')[0]

	# Print Section 5 raw metadata array:
	print("Section 5 contents:", precip_start_msg.section5)

	# Or inspect individual Data Representation attributes if available:
	if hasattr(precip_start_msg, 'decimal_scale_factor'):
	    print("Decimal Scale Factor:", precip_start_msg.decimal_scale_factor)
	elif hasattr(precip_start_msg, 'dec_scale_factor'):
	    print("Decimal Scale Factor:", precip_start_msg.dec_scale_factor)

	tprate_start = np.nan_to_num(precip_start_msg.data, nan=0.0)
	precip_start = tprate_start * 0.0393701   # Convert mm to inches

	print(f"Extracted: {precip_start_msg.shortName} for {precip_start_msg.leadTime} hours")
	print(f"Max Precip: {precip_start.max():.2f} in.")

filename_ecmwf_clean = get_uncompressed_grib(filename_ecmwf)

with grib2io.open(filename_ecmwf_clean) as f_ecmwf:

	precip_msg = f_ecmwf.select(shortName='TPRATE')[0]
	tprate = np.nan_to_num(precip_msg.data, nan=0.0)
	precip_end = tprate * 0.0393701   # Convert mm to inches

	print(f"Extracted: {precip_msg.shortName} for {precip_msg.leadTime} hours")
	print(f"Max Precip: {precip_end.max():.2f} in.")

	# Calculate the difference (e.g., APCP at end - APCP at start)
	diff_data = (precip_end - precip_start)

	# Clamp tiny negative values from AI model noise to zero
	diff_data = np.maximum(diff_data, 0.0)
...
print(f"The minimum precip is: {minimum} in.")
print(f"The maximum precip is: {maximum} in.")
...
plt.tight_layout()
plt.savefig(f"{MAP_PATH}/{grid}/{var}/ecaifs_{duration}h{var}_init{pdy}_{cyc}Z_f{fhr}_valid{valid_dt.strftime('%Y%m%d')}_{valid_dt.strftime('%H')}Z.png", bbox_inches='tight', pad_inches=0.1)
