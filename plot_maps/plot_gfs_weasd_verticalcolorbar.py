...
for i, loc in enumerate(grid_locs):
	...
	cbar = plt.colorbar(im, ax=ax, ticks=snod_levels, orientation='vertical', pad=0.06, fraction=0.055, shrink=0.95) # fraction is height, shrink is width
...
plt.tight_layout()
plt.savefig(f"{MAP_PATH}/{grid}/{var}/gfs_{var}_init{pdy}_{cyc}Z_f{fhr}_verticalcolorbar.png", bbox_inches='tight', pad_inches=0.1)
