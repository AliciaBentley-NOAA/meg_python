...
# Open HGEFS GRIB2 file and extract parameters
filename_hgefss = f"{DATA_PATH}/hgefs.{pdy}/{cyc}/atmos/hgefs.t{cyc}z.pres.spr.f{fhr_str}.grib2"
with grib2io.open(filename_hgefss) as f_hgefss:

        # Select the specific messages we want
        hgt500s_msg = f_hgefss.select(shortName='HGT', level='500 mb')[0]
...
plot_configs = [
	{'data': hgt500_data, 'cmap': 'YlOrRd', 'norm': hgt500s_norm, 'levels': hgt500s_levels, 'title': f'HGEFS mean/spread | 500-hPa Geopotential Height (dam)\nInitialized: {init_dt.strftime("%Y-%m-%d %HZ")} (F{fhr_str}) | Valid: {valid_dt.strftime("%Y-%m-%d %HZ")}'},
]
