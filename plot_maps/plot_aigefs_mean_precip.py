...
print(f"Number of AIGEFS files needed: {segments}")

aigefs_array = np.zeros((int(segments), 721, 1440), dtype=np.float32)

for j in range(int(segments)):

    # Remember: Valid time in filenames is at the *end* of the 6-h period (add gefs_delta)
    current_dt = current_dt + gefs_delta
    print(f"Current AIGEFS File Time: {current_dt.strftime('%Y-%m-%d %HZ')}")
...
plot_configs = [
	{'data': diff_data, 'cmap': cmap, 'norm': norm, 'levels': precip_levels, 'title': f'AIGEFS mean | {duration}-h Accumulated Precipitation (in.)\nInitialized: {init_dt.strftime("%Y-%m-%d %HZ")} (F{fhr_str}) | Valid: {valid_dt.strftime("%Y-%m-%d %HZ")}'},
]
...
plt.tight_layout()
plt.savefig(f"{MAP_PATH}/{grid}/{var}/aigefs_{var}_init{pdy}_{cyc}Z_f{fhr}.png", bbox_inches='tight', pad_inches=0.1)
