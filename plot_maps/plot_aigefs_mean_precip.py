...
for j in range(int(segments)):
    ...
    filename_gefs = f"{DATA_PATH}/aigefs.{pdy}/{cyc}/atmos/aigefs.t{cyc}z.sfc.avg.f{fhr_str}.grib2"
    print(filename_gefs)
...
plot_configs = [
	{'data': diff_data, 'cmap': cmap, 'norm': norm, 'levels': snod_levels, 'title': f'AIGEFS mean | {duration}-h Accumulated Precipitation (in.)\nInitialized: {init_dt.strftime("%Y-%m-%d %HZ")} (F{fhr_str}) | Valid: {valid_dt.strftime("%Y-%m-%d %HZ")}'},
]
