...
with grib2io.open(filename_gfs) as f_gfs:
	...
	mr_msg = f_gfs.select(shortName='GRLE', level='1 hybrid level')[0]

	# Extract values
	mr_data = mr_msg.data * 1000.0  # Convert kg to g

	lats, lons = mr_msg.latlons()
...
#mr_norm = mcolors.Normalize(vmin=0, vmax=24)
#mr_levels = np.arange(0, 25, 1)
mr_levels = np.array([0.01, 0.025, 0.075, 0.1, 0.25, 0.5, 0.75, 1.0, 2.0])

#snod_colors = ['#749DDE', '#588ADC', '#2F74C8', '#2364B9', '#1E559D', '#FFF68F', '#F4C430', '#ED781E', '#E23916', '#C92828', '#D986D9', '#D95DD9']
mr_colors = ['#749DDE', '#588ADC', '#2F74C8', '#88db8e', '#51c458', '#1fa328', '#F4C430', '#ED781E']

cmap = mcolors.ListedColormap(mr_colors)
#cmap.set_under('white')
cmap.set_over('red')

norm = mcolors.BoundaryNorm(mr_levels, ncolors=len(mr_colors))

# Update configs with specific 'norm' and 'levels'
plot_configs = [
	{'data': mr_data, 'cmap': cmap, 'norm': norm, 'levels': mr_levels, 'title': f'GFSv16 | Graupel Mixing Ratio at 1 Hybrid Level (g/kg)\nInitialized: {init_dt.strftime("%Y-%m-%d %HZ")} (F{fhr_str}) | Valid: {valid_dt.strftime("%Y-%m-%d %HZ")}'},
]
...
for i, loc in enumerate(grid_locs):
	...
	# Geographic features
    ax.add_feature(cfeature.STATES, edgecolor='0.25', linewidth=2.0, zorder=3.0)
    ax.add_feature(cfeature.COASTLINE, edgecolor='0.25', linewidth=1.5, zorder=3.0)
    ax.add_feature(cfeature.BORDERS, edgecolor='0.25', linewidth=1.5, zorder=3.0)
    ax.add_feature(cfeature.LAKES, facecolor='white', edgecolor='0.25', linewidth=1.0, zorder=1.0)
    ...
    cbar = plt.colorbar(im, ax=ax, ticks=mr_levels, orientation='horizontal', pad=0.06, fraction=0.055, shrink=0.95) # fraction is height, shrink is width
    ...
    cbar.ax.set_xticklabels([f'{l:g}' for l in mr_levels])
