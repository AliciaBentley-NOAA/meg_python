...
filename_hgefs = f"{DATA_PATH}/hgefs.{pdy}/{cyc}/atmos/hgefs.t{cyc}z.pres.avg.f{fhr_str}.grib2"
with grib2io.open(filename_hgefs) as f_hgefs:

	# Select the specific messages we want
	hgt500_msg = f_hgefs.select(shortName='HGT', level='500 mb')[0]
...
plot_configs = [
	{'data': hgt500_data, 'cmap': 'gist_rainbow_r', 'norm': hgt500_norm, 'levels': hgt500_levels, 'title': f'HGEFS mean 500-hPa Geopotential Height (dam)\nInitialized: {init_dt.strftime("%Y-%m-%d %HZ")} (F{fhr_str}) | Valid: {valid_dt.strftime("%Y-%m-%d %HZ")}'},
]
...
#plt.suptitle(f"HGEFS mean | 500-hPa Geopotential Height (dam) | Initialized: {init_dt.strftime('%Y-%m-%d %HZ')} (Fhr: {fhr_str}) | Valid: {valid_dt.strftime('%Y-%m-%d %HZ')}", fontsize=20)
plt.tight_layout()
plt.savefig(f"{MAP_PATH}/{grid}/{var}/hgefs_mean_{var}_init{pdy}_{cyc}Z_f{fhr}.png", bbox_inches='tight', pad_inches=0.1)
