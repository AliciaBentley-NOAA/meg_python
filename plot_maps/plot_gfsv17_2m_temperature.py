...
#temp_norm = mcolors.Normalize(vmin=-24, vmax=120)
#temp_levels = np.arange(-24, 122, 2)
...
plot_configs = [
	{'data': temp_data, 'cmap': 'gist_rainbow_r', 'norm': temp_norm, 'levels': temp_levels, 'title': f'GFSv17 2-m Temperature (F)\nInitialized: {init_dt.strftime("%Y-%m-%d %HZ")} (F{fhr_str}) | Valid: {valid_dt.strftime("%Y-%m-%d %HZ")}'},
]
