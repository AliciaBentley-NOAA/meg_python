...
# Container to accumulate PRMSL data across all ensemble members
member_mslp_list = []
lats, lons = None, None

# Loop through AIGEFS members mem000 to mem030
for m in range(31):
  mem_str = f"mem{m:03d}"
  filename_aigefs = f"{DATA_PATH}/aigefsdev.{pdy}/{cyc}/atmos/{mem_str}/aigefs.t{cyc}z.sfc.f{fhr_str}.grib2"

  try:
    with grib2io.open(filename_aigefs) as f_aigefs:
      mslp_msg = f_aigefs.select(shortName="PRMSL", level="mean sea level")[0]

      # Convert Pa to hPa/mb and append to list
      mslp_data = mslp_msg.data / 100.0
      member_mslp_list.append(mslp_data)

      # Grab spatial coordinates once on the first successful read
      if lats is None or lons is None:
        lats, lons = mslp_msg.latlons()

  except (FileNotFoundError, IndexError) as e:
    print(f"Warning: Skipping {mem_str} due to read error: {e}")

# Compute the ensemble mean across the member axis (axis 0)
if member_mslp_list:
  # Stacks list of 2D arrays into a 3D array of shape (N_members, lat, lon)
  mslp_ensemble_stack = np.array(member_mslp_list)

  # Calculate ensemble mean
  mslp_mean = np.mean(mslp_ensemble_stack, axis=0)

  print(
      f"Successfully averaged {len(member_mslp_list)} members. Mean shape:"
      f" {mslp_mean.shape}"
  )
else:
  raise ValueError("No valid member files were successfully read!")
...
plot_configs = [
        {'data': mslp_data, 'cmap': 'gist_rainbow', 'norm': mslp_norm, 'levels': mslp_levels, 'title': f'AIGEFS-p2.1 Mean | PRMSL - Mean Sea Level Pressure (hPa)\nInitialized: {init_dt.strftime("%Y-%m-%d %HZ")} (F{fhr_str}) | Valid: {valid_dt.strftime("%Y-%m-%d %HZ")}'},
]
...
plt.tight_layout()
plt.savefig(f"{MAP_PATH}/{grid}/{var}/aigefsdev_mean_{var}_init{pdy}_{cyc}Z_f{fhr}.png", bbox_inches='tight', pad_inches=0.1)
