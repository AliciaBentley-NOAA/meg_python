...
for mem in range(31):
    mem_str = "gec00" if mem == 0 else f"gep{mem:02d}"
    
    filename_gefs = f"{DATA_PATH}/gefs.{pdy}/{cyc}/atmos/{mem_str}.t{cyc}z.pgrb2s.0p25.f{fhr_str}"
    
    with grib2io.open(filename_gefs) as f_gefs:
        mslp_mem_msg = f_gefs.select(shortName='PRMSL', level='mean sea level')[0]
        mslp_mem_data = mslp_mem_msg.data / 100.0  # Convert Pa to hPa/mb
        mslp_list.append(mslp_mem_data)

        # Get 0.25 degree lats and lons (different from mean/spread)
        lats_mem, lons_mem = mslp_mem_msg.latlons()
...
# Shift longitudes from [0, 360] to [-180, 180]
lons_mem = np.where(lons_mem > 180, lons_mem - 360, lons_mem)
                
# If lons is a 2D meshgrid, we only want the 1D vector to get sort indices
# We'll take the first row of lons to determine the sorting order
if lons_mem.ndim == 2:
        lons_mem_1d = lons_mem[0, :]
else:
        lons_mem_1d = lons_mem
        
# Get the sorting indices
i_sort = np.argsort(lons_mem_1d)

# Apply sorting to the 2D arrays across the longitude axis (axis=1)
if lons_mem.ndim == 2:
        lons_mem = lons_mem[:, i_sort]
        lats_mem = lats_mem[:, i_sort] # Sort lats too if it's a meshgrid
else:
        lons_mem = lons_mem[i_sort]
...
for i, loc in enumerate(grid_locs):
	...
	for i in range(num_members):
		ax.contour(lons_mem, lats_mem, mslp_ensemble[i],
			levels = [1004],
			colors='blue',
			linewidths=2.2,
			alpha=0.6,
...
plt.tight_layout()
plt.savefig(f"{MAP_PATH}/{grid}/{var}/gefs_members_{var}_init{pdy}_{cyc}Z_f{fhr}.png", bbox_inches='tight', pad_inches=0.1)
