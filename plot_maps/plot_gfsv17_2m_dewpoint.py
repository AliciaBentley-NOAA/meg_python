...
with grib2io.open(filename_gfsv17) as f_gfsv17:
    ...
    temp_msg = f_gfsv17.select(shortName='DPT', level='2 m above ground')[0]
...
plot_configs = [
    {'data': temp_data, 'cmap': cmap, 'norm': temp_norm, 'levels': temp_levels, 'title': f'GFSv17 2-m Dewpoint (F)\nInitialized: {init_dt.strftime("%Y-%m-%d %HZ")} (F{fhr_str}) | Valid: {valid_dt.strftime("%Y-%m-%d %HZ")}'},
]
