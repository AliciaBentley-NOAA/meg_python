...
with grib2io.open(filename_gefs) as f_gefs:
	...
	mslp_msg = f_gefs.select(shortName='MSLET', level='mean sea level')[0]
...
plot_configs = [
        {'data': mslp_data, 'cmap': 'gist_rainbow', 'norm': mslp_norm, 'levels': mslp_levels, 'title': f'GEFS Mean | MSLET - Mean Sea Level Pressure (hPa)\nInitialized: {init_dt.strftime("%Y-%m-%d %HZ")} (F{fhr_str}) | Valid: {valid_dt.strftime("%Y-%m-%d %HZ")}'},
]
