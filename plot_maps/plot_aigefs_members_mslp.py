...
# Container to store 2D MSLP arrays for each member
mslp_list = []

# Loop through all 31 members (0 = control, 1-30 = perturbed)
for mem in range(31):
    mem_str = f"mem{mem:03d}"
    
    filename_aigefs = f"{DATA_PATH}/aigefs.{pdy}/{cyc}/atmos/{mem_str}_aigefs.t{cyc}z.sfc.f{fhr_str}.grib2"
    
    with grib2io.open(filename_aigefs) as f_aigefs:
        mslp_mem_msg = f_aigefs.select(shortName='PRMSL', level='mean sea level')[0]
        mslp_mem_data = mslp_mem_msg.data / 100.0  # Convert Pa to hPa/mb
        mslp_list.append(mslp_mem_data)

# Stack into a 3D NumPy array with shape: (31, lat_size, lon_size)
mslp_ensemble = np.array(mslp_list)

# Open AIGEFS GRIB2 file and extract parameters
filename_aigefs = f"{DATA_PATH}/aigefs.{pdy}/{cyc}/atmos/aigefs.t{cyc}z.sfc.avg.f{fhr_str}.grib2"
with grib2io.open(filename_aigefs) as f_aigefs:

	# Select the specific messages we want
	mslp_msg = f_aigefs.select(shortName='PRMSL', level='mean sea level')[0]
...
# Open AIGEFS GRIB2 file and extract parameters
filename_aigefss = f"{DATA_PATH}/aigefs.{pdy}/{cyc}/atmos/aigefs.t{cyc}z.sfc.spr.f{fhr_str}.grib2"
with grib2io.open(filename_aigefss) as f_aigefss:

        # Select the specific messages we want
        mslps_msg = f_aigefss.select(shortName='PRMSL', level='mean sea level')[0]
...
mslp_ensemble = mslp_ensemble[:, :, i_sort]
...
plot_configs = [
	{'data': mslp_data, 'cmap': 'YlOrRd', 'norm': mslps_norm, 'levels': mslps_levels, 'title': f'AIGEFS mean/spread | Mean Sea Level Pressure (hPa)\nInitialized: {init_dt.strftime("%Y-%m-%d %HZ")} (F{fhr_str}) | Valid: {valid_dt.strftime("%Y-%m-%d %HZ")}'},
]
...
for i, loc in enumerate(grid_locs):
	...
	num_members = mslp_ensemble.shape[0]
	for i in range(num_members):
		ax.contour(lons, lats, mslp_ensemble[i],
			levels = [1004],
			colors='blue',
			linewidths=2.2,
			alpha=0.6,
...
plt.tight_layout()
plt.savefig(f"{MAP_PATH}/{grid}/{var}/aigefs_members_{var}_init{pdy}_{cyc}Z_f{fhr}.png", bbox_inches='tight', pad_inches=0.1)
