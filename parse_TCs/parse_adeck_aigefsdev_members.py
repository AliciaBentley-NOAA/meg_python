...
DIR = os.getcwd()
ADECK_DIR = '/lfs/h2/emc/vpppg/noscrub/alicia.bentley/adecks_aigefsdev'
...
for m in range(31):
   ...
   with open(ADECK_DIR+'/a'+str.lower(TC_number)+YYYY+'_'+mem_str+'.dat','r') as f:
      reader = csv.reader(f)
      for row in reader:
...
#           rmw.append(row[19].replace(" ",""))
   ...
   for cycle in cycles_in_file:
      ...
      f = open(DATA_DIR+'/'+str.lower(TC_name)+'_'+str.lower(model_str)+'_'+cycle+'_'+mem_str+'.csv','wt')

      cycle_time = datetime.datetime(int(cycle[0:4]),int(cycle[4:6]),int(cycle[6:8]),int(cycle[8:10]))
      #print(f"len(cycles): {len(cycles)}")

      # Catch up i if cycles[i] is behind the current file's cycle
      while i < len(cycles) and cycles[i] < cycle:
           i += 1

      # Stop if we hit the end of the dataset
      if i >= len(cycles):
           break

      try:
         writer = csv.writer(f)
         print(f"File cycle: {cycle} | Data cycle: {cycles[i]} | i: {i}")

         # Write matching rows while guarding against index bounds
         while i < len(cycles) and cycle == cycles[i]:
