...
mkdir -p ${DATA_PATH}/untar_fcsts/untar_gefs_mem_p18/gefs.${YYYYMMDD}/${HH}
...
if ((${CYCLE} >= ${GEFS_CHANGE_DATE6})) ; then
        ...
        GEFS_FILENAME=./atmos/pgrb2sp25/gep18.t${HH}z.pgrb2s.0p25.f${FHHH_same}
...
cat > ${DATA_PATH}/untar_fcsts/untar_gefs_mem_p18/gefs.${YYYYMMDD}/${HH}/htar_gefs_mem_p18_fcst_atmos.sh <<EOF
#!/bin/bash
#PBS -N gefs_mem_p18_htar
#PBS -o ${OUTPUT_PATH}/out_htar_gefs_mem_p18_fcst_atmos_${YYYYMMDD}_${HH}.out
#PBS -e ${OUTPUT_PATH}/out_htar_gefs_mem_p18_fcst_atmos_${YYYYMMDD}_${HH}.err
#PBS -l select=1:ncpus=1:mem=4GB
#PBS -q dev_transfer
#PBS -l walltime=02:00:00
#PBS -A VERF-DEV

cd ${DATA_PATH}/untar_fcsts/untar_gefs_mem_p18/gefs.${YYYYMMDD}/${HH}
...
while IFS= read -r line ; do
	...
	if [[ -s ${DATA_PATH}/gefs.${YYYYMMDD}/${HH}/atmos/gep18.t${HH}z.pgrb2s.0p25.f${FHHH_same} ]] ; then
		echo ${CYCLE} "F"${FHHH_same}" GEFS hires (gep18) member forecast already exists"
	else
        	echo "Extracting "${CYCLE}" ops GEFS hires (gep18) member forecast file "${FHHH_same}
        	htar -xvf $GEFS_ARCHIVE $GEFS_FILENAME
        	sleep 1
        	mv $GEFS_FILENAME ${DATA_PATH}/gefs.${YYYYMMDD}/${HH}/atmos/gep18.t${HH}z.pgrb2s.0p25.f${FHHH_same}
	fi
...
qsub ${DATA_PATH}/untar_fcsts/untar_gefs_mem_p18/gefs.${YYYYMMDD}/${HH}/htar_gefs_mem_p18_fcst_atmos.sh
sleep 1
