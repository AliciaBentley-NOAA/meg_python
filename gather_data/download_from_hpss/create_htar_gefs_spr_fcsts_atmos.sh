...
mkdir -p ${DATA_PATH}/untar_fcsts/untar_gefs_spr/gefs.${YYYYMMDD}/${HH}
...
if ((${CYCLE} >= ${GEFS_CHANGE_DATE6})) ; then
        ...
        GEFS_FILENAME=./atmos/pgrb2sp25/gespr.t${HH}z.pgrb2s.0p25.f${FHHH_same}
...
cat > ${DATA_PATH}/untar_fcsts/untar_gefs_spr/gefs.${YYYYMMDD}/${HH}/htar_gefs_spr_fcst_atmos.sh <<EOF
#!/bin/bash
#PBS -N gefs_spr_htar
#PBS -o ${OUTPUT_PATH}/out_htar_gefs_spr_fcst_atmos_${YYYYMMDD}_${HH}.out
#PBS -e ${OUTPUT_PATH}/out_htar_gefs_spr_fcst_atmos_${YYYYMMDD}_${HH}.err
#PBS -l select=1:ncpus=1:mem=4GB
#PBS -q dev_transfer
#PBS -l walltime=02:00:00
#PBS -A VERF-DEV

cd ${DATA_PATH}/untar_fcsts/untar_gefs_spr/gefs.${YYYYMMDD}/${HH}
...
while IFS= read -r line ; do
	...
	if [[ -s ${DATA_PATH}/gefs.${YYYYMMDD}/${HH}/atmos/gespr.t${HH}z.pgrb2s.0p25.f${FHHH_same} ]] ; then
		echo ${CYCLE} "F"${FHHH_same}" GEFS hires spread forecast (gespr) already exists"
	else
        	echo "Extracting "${CYCLE}" ops GEFS hires spread forecast (gespr) file "${FHHH_same}
        	htar -xvf $GEFS_ARCHIVE $GEFS_FILENAME
        	sleep 3
        	mv $GEFS_FILENAME ${DATA_PATH}/gefs.${YYYYMMDD}/${HH}/atmos/gespr.t${HH}z.pgrb2s.0p25.f${FHHH_same}
	fi
...
qsub ${DATA_PATH}/untar_fcsts/untar_gefs_spr/gefs.${YYYYMMDD}/${HH}/htar_gefs_spr_fcst_atmos.sh
sleep 3
