...
mkdir -p ${DATA_PATH}/gfs.${YYYYMMDD}/${HH}/products/atmos/grib2/0p25
#echo ${DATA_PATH}/gfs.${YYYYMMDD}/${HH}/atmos
...
if ((${CYCLE} >= ${GFS_CHANGE_DATE6})) ; then
        GFS_ARCHIVE=/5year/NCEPDEV/emc-global/emc.glopara/WCOSS2/GFSv17/retrov17_01_realtime/${YYYYMMDD}${HH}/gfsa.tar
        GFS_FILENAME=gfs.${YYYYMMDD}/${HH}/products/atmos/grib2/0p25/gfs.t${HH}z.pres_a.0p25.f${FHHH_same}.grib2
...
cat > ${DATA_PATH}/untar_fcsts/untar_gfs/gfs.${YYYYMMDD}/${HH}/htar_gfs_fcst_atmos.sh <<EOF
#!/bin/bash
#PBS -N gfs_htar
#PBS -o ${OUTPUT_PATH}/out_htar_gfs_fcst_atmos_${YYYYMMDD}_${HH}.out
#PBS -e ${OUTPUT_PATH}/out_htar_gfs_fcst_atmos_${YYYYMMDD}_${HH}.err
...
while IFS= read -r line ; do
	...
	if [[ -s ${DATA_PATH}/gfs.${YYYYMMDD}/${HH}/products/atmos/grib2/0p25/gfs.t${HH}z.pres_a.0p25.f${FHHH_same}.grib2 ]] ; then
		echo ${CYCLE} "F"${FHHH_same}" GFS forecast already exists"
	else
        	echo "Extracting "${CYCLE}" ops GFS forecast file "${FHHH_same}
        	htar -xvf $GFS_ARCHIVE $GFS_FILENAME
        	sleep 3
        	mv $GFS_FILENAME ${DATA_PATH}/gfs.${YYYYMMDD}/${HH}/products/atmos/grib2/0p25/gfs.t${HH}z.pres_a.0p25.f${FHHH_same}.grib2
	fi
...
qsub ${DATA_PATH}/untar_fcsts/untar_gfs/gfs.${YYYYMMDD}/${HH}/htar_gfs_fcst_atmos.sh
sleep 3
